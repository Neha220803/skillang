...
import { Button, Card, Form, Container, Row, Col, Alert, Spinner, Table, InputGroup, Tabs, Tab, Badge } from 'react-bootstrap';
...
const Mailer = () => {
    ...
    const fetchScheduledEmails = async () => {
        ...
        try {
            const response = await axios.get('https://skillang.com/api/api/scheduled-emails'); //http://localhost:3001/api/scheduled-emails
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const response = await axios.post('https://skillang.com/api/api/send-template', payload);

            setSuccess(response.data.message || (scheduleEmail ? 'Emails scheduled!' : 'Emails sent!'));
    ...
    const handleCancelScheduledEmail = async (id) => {
        try {
            setCancellingId(id);
            const response = await axios.delete(`https://skillang.com/api/api/scheduled-emails/${id}`);
        ...
        <Container className="mt-5 mb-5">
            <Row className="justify-content-md-center">
                <Col md={10}>
                    <Card>
                        <Card.Header as="h5">
                            {isAuthenticated ? 'Email Campaign Manager' : 'Login to Access Mailer'}
                        </Card.Header>
                        <Card.Body>
                            {!isAuthenticated ? (
                                <Form>
                                    ...
                                    >
                                        <Tab eventKey="composer" title="Email Composer">
                                            <Form>
                                                ...
                                                <Form.Group controlId="formScheduleToggle" className="mb-3">
                                                    <Form.Check
                                                        type="switch"
                                                ...
                                                {scheduleEmail && (
                                                    <Form.Group controlId="formScheduleTime" className="mb-3">
                                                        ...
                                                        <Form.Text className="text-muted">
                                                            Time must be in the future. Current server time zone is IST (Indian Standard Time).
                                                        </Form.Text>
                                                    </Form.Group>
                                                )}
                                                ...
                                                <Form.Group controlId="formFile" className="mb-3">
                                                    ...
                                                    />
                                                    <Form.Text className="text-muted">
                                                        CSV must have 'email' and 'username' columns. Recipients will be added to the existing list.
                                                    </Form.Text>
                                                </Form.Group>
                                                ...
                                                <Button
                                                    ...
                                                    disabled={
                                                        ...
                                                        !senderEmailPrefix ||
                                                        !templateKey ||
                                                        (scheduleEmail && (!scheduledTime || !isFutureDate(scheduledTime)))
                                                    }
                                                >
                                                    {loading ? (
                                                        <>
                                                            <Spinner animation="border" size="sm" /> {scheduleEmail ? 'Scheduling...' : 'Sending...'}
                                                        </>
                                                    ) : (
                                                        `${scheduleEmail ? 'Schedule' : 'Send'} ${recipients.length} Email${recipients.length > 1 ? 's' : ''}`
                                                    )}
                                                </Button>
                                            </Form>
                                        </Tab>
                                        ...
                                        <Tab eventKey="scheduled" title="Scheduled Emails">
                                            <div className="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <ul className="nav nav-tabs w-100">
                                                    <li className="nav-item" style={{ width: '20%' }}>
                                                        <a
                                                            className={`nav-link text-center ${statusFilter === 'all' ? 'active bg-primary text-white' : ''}`}
                                                            href="#"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                setStatusFilter('all');
                                                    ...
                                                    <li className="nav-item" style={{ width: '20%' }}>
                                                        <a
                                                            className={`nav-link text-center ${statusFilter === 'pending' ? 'active bg-warning text-dark' : ''}`}
                                                            href="#"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                    ...
                                                    <li className="nav-item" style={{ width: '20%' }}>
                                                        <a
                                                            className={`nav-link text-center ${statusFilter === 'sent' ? 'active bg-success text-white' : ''}`}
                                                            href="#"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                    ...
                                                    <li className="nav-item" style={{ width: '20%' }}>
                                                        <a
                                                            className={`nav-link text-center ${statusFilter === 'failed' ? 'active bg-danger text-white' : ''}`}
                                                            href="#"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                    ...
                                                    <li className="nav-item" style={{ width: '20%' }}>
                                                        <a
                                                            className={`nav-link text-center ${statusFilter === 'cancelled' ? 'active bg-secondary text-white' : ''}`}
                                                            href="#"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                            ...
                                            {loadingScheduled ? (
                                                ...
                                                <Alert variant="info">
                                                    No scheduled emails found. Use the Email Composer tab to schedule emails.
                                                </Alert>
                                            ) : (
                                                <>
                                                                ...
                                                                {scheduledEmails
                                                                    ...
                                                                    .map((email) => (
                                                                        <tr key={email._id}>
                                                                            ...
                                                                            <td>{formatDateTime(email.scheduledTime)}</td>
                                                                            <td>
                                                                                <Badge bg={getStatusBadgeVariant(email.status)}>
                                                                                    {email.status}
                                                                                </Badge>
                                                                            </td>
                                                                            <td>
                                                                                ...
                                                                                {email.status === 'failed' && (
                                                                                    <span className="text-danger small" title={email.error}>
                                                                                        Error: {email.error ? email.error.substring(0, 20) + '...' : 'Unknown'}
                                                                                    </span>
                                                                                )}
                                                                            </td>
                                                                        </tr>
                                                    ...
                                                    {/* See More button */}
                                                    {displayCount < scheduledEmails.filter(email => statusFilter === 'all' || email.status === statusFilter).length && (
                                                    ...
                                                    <div className="mt-3 small text-muted">
                                                        Showing {Math.min(displayCount, scheduledEmails.filter(email => statusFilter === 'all' || email.status === statusFilter).length)} of {scheduledEmails.filter(email => statusFilter === 'all' || email.status === statusFilter).length} emails

...
const ConsultationModal = ({ show, handleClose }) => {
  ...
  // Keep original useEffect but modify it to skip OTP for now
  useEffect(() => {
    if (formIsValid && isOtpSent) {
      // Original behavior - uncomment when needed
      // setCurrentStep(2);

      // Temporary behavior - skip OTP verification and go directly to Calendly step
      setCurrentStep(3);
    }
  }, [formIsValid, isOtpSent]);

  // Add this useEffect to handle OTP verification success - commented out for now
  // useEffect(() => {
  //   if (isOtpVerified) {
  ...
  // Modify the form submission handler temporarily while preserving original functionality for later
  const handleModalFormSubmit = (e) => {
    ...
    if (
      ...
      // Comment out code that depends on OTP verification
      // Normal flow would wait for isOtpSent to become true and then change step in useEffect
      // For now, we'll set step directly here to bypass OTP screen
      // This will be removed when OTP verification is needed again
      setCurrentStep(3);
    }
  };
